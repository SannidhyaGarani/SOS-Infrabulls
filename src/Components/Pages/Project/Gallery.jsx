...
const ProjectCard = ({ project, onView }) => (
  <article className="group bg-white rounded-2xl overflow-hidden shadow-md hover:-translate-y-2 hover:shadow-[0_24px_60px_rgba(17,116,214,0.14)] transition-all duration-400 flex flex-col border border-slate-100">
  ...
  useEffect(() => {
    const unsub = onSnapshot(collection(db, "projects"),
      (snap) => {
        ...
        setProjects(data.filter((p) => (p.status || "running") === "running"));
        setLoading(false);
      },
      () => { setError("Unable to load running projects."); setLoading(false); }
    );
    return () => unsub();
  }, []);
  ...
  return (
    <div className="min-h-screen bg-white">
      <ImageBreadcrumb
        ...
        subtitle="Live developments where your dream investment awaits — actively selling now."
        crumbs={[{ label: 'Projects', href: '/projects' }, { label: 'Ongoing' }]}
      />

      <section className="py-20">
        <div className="max-w-7xl mx-auto px-6 lg:px-12">
          <div className="text-center mb-14">
            <p className="text-[#1174d6] text-xs font-bold tracking-widest uppercase mb-3">Live Developments</p>
            <h2
              className="text-slate-900 font-light"
              style={{ fontFamily: "'Playfair Display', Georgia, serif", fontSize: "clamp(1.8rem, 3vw, 2.8rem)" }}
            >
              Running <strong className="font-bold">Project Gallery</strong>
            </h2>
          </div>
          ...
          {loading ? (
            ...
            <div className="text-center py-24">
              ...
              <p className="text-slate-400 text-sm">Running projects will appear here soon.</p>
            </div>
          ) : (
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {projects.map((p) => (
